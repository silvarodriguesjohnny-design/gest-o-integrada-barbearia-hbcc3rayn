...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === 'GET') return new Response(JSON.stringify({ status: 'ok' }), { headers: { 'Content-Type': 'application/json', ...corsHeaders } })
  ...
  try {
    ...
    if (!phone || !message) {
      return new Response(JSON.stringify({ error: 'Missing phone or message' }), { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } })
    }

    let { data: customer } = await supabase.from('customers').select('*').eq('phone', phone).limit(1).maybeSingle()
    ...
    if (message.includes('agendar') || message.includes('horario') || message.includes('marcar')) {
      const { data: services } = await supabase.from('services').select('*').eq('tenant_id', tid).order('name')
      const slots: string[] = []
      for (let d = 0; d < 3 && slots.length < 6; d++) {
        ...
        const bookedHours = new Set((booked || []).map((a: any) => new Date(a.start_time).getHours()))
        for (let h = 9; h < 18 && slots.length < 6; h++) {
          if (!bookedHours.has(h)) {
            slots.push(`${date.toLocaleDateString('pt-BR', { weekday: 'short', day: '2-digit', month: '2-digit' })} às ${String(h).padStart(2, '0')}:00`)
          }
        }
      }
      const svcList = (services || []).map((s: any, i: number) => `${i + 1}. ${s.name} - R$ ${s.price}`).join('\n')
      responseText = slots.length > 0
      ...
      if (services && idx >= 0 && idx < services.length) {
        ...
        const { error } = await supabase.from('appointments').insert({
          customer_id: customer?.id, service_id: svc.id, barber_name: 'A definir',
          status: 'scheduled', start_time: start.toISOString(), end_time: end.toISOString(), tenant_id: tid,
        })
        responseText = error
    ...
    return new Response(JSON.stringify({ success: true, response: responseText, customer_id: customer?.id }), {
